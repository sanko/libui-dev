...
uiDragData *uiDragContextDragData(uiDragContext *dc, uiDragType type)
{
	...
	if (!(dc->dd->typeMask & type)) {
		uiprivUserBug("You requested uiDragType (%d) which is not accepted by the drag destination", type);
		return NULL;
	}
	...
	case uiDragTypeText:
		{
			...
			if (pDataObj->GetData(&fmtetc, &medium) == S_OK) {
				...
				d->data.text = (char *) uiprivAlloc(len + 1, "uiDragData->data.text");
				memcpy(d->data.text, data, len + 1);
				GlobalUnlock(data);
			}
		}
		break;
